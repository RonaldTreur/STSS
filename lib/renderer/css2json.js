...
var parseCSS = require('css-parse'),
	...
	reTitaniumConstant = /^(?:ti[.])|(?:titanium[.])|(?:alloy[.])/i,
	fileProperties = [
		'backgroundImage',
...
function loadShorthand(options) {
	...
	shorthand.replacers || (shorthand.replacers = {});
	...
	customShorthand.replacers || (customShorthand.replacers = {});
}
 ...
 * @param  {Object} special	Special STSS rules parsed from the CSS
 * @return {String}       	The property's name if this declaration's value needs to be unquoted in TSS, null otherwise
 */
function parseDeclaration(decl, output, special) {
	...
	// If value is an i18N command: Unquote it
	...
	// If the value is shortened: Expand it
		...
		value = value.replace(/^(["'])(\w+(?=\.)[\w.]+)\1$/g, function(match, quote, val) {
			...
			// If value is a filename (or could be one), ignore
			} else if (fileProperties.indexOf(prop) !== -1 && !reTitaniumConstant.test(val)) {
				return match;
			} else {
				unquote = true;
				return val;
