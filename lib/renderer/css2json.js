...
function parseSpecialValue(value, special) {
	...
	if (type === 'array') {
		...
		var arrayNr = matches[1],
			valueString = matches[2],
			values = [];

		valueString.replace(/stss-array(\d+)-val(\d+)/g, function(m, aNr, valueNr) {
			// TODO? Sanity check aNr === arrayNr
			values.push(special['array'][arrayNr][valueNr][0].body);
		});
		return values;
	}
...
function parseDeclaration(decl, output, special) {
	...
	} else if (1*value - parseFloat(value) >= 0) {
		if (/\./.test(value)) {
	...
	// If value is a i8N command: Unquote it
	...
	// If the value is shortened: Expand it
		...
		// Unquote values that contain no whitespaces, but do contain dots (probably Ti & Alloy structures)
...
function parseAST(rules) {
	...
	for (i = rules.length-1; i > -1; i--) {
		...
		if (rule.type === "rule" &&
			...
			matches = /-stss-([a-z]+)(\d+)-val(\d+)/.exec(rule.selectors[0]);
