...
function parseDeclaration(decl, output) {
	...
	if (parent && shorthand.propertyNames[parent] && shorthand.propertyNames[parent][prop]) {
		prop = shorthand.propertyNames[parent][prop];
	}
	...
	} else if (shorthand.propertyValues[prop] && shorthand.propertyValues[prop][value]) {
		value = shorthand.propertyValues[prop][value];
